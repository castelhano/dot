...
class OcorrenciaForm(forms.ModelForm):
    ...
    evento = forms.ModelChoiceField(error_messages={'required': 'Selecione uma ocorrencia'}, queryset = Evento.objects.all().order_by('nome'), widget=forms.Select(attrs={'class':'form-select fw-bold'}))
    ...
    detalhe = forms.CharField(required=False, widget=forms.Textarea(attrs={'class': 'form-control','placeholder':'Detalhes','style':'min-height:350px;'}))
...
class AgenteForm(forms.ModelForm):
    ...
    orgao = forms.ModelChoiceField(error_messages={'required': 'Informe o nome do orgão'}, queryset = Orgao.objects.all().order_by('nome'), widget=forms.Select(attrs={'class':'form-select', 'autofocus':'autofocus'}))
...
class NotificacaoForm(forms.ModelForm):
    ...
    tipo = forms.ChoiceField(choices=Notificacao.TIPO_CHOICES, widget=forms.Select(attrs={'class':'form-select fw-bold'}))
